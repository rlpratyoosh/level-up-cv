...
export default function Certifications({ profile, profileLoading }: CertificationsProps) {
    return (
        <div className="flex-1 p-7 flex flex-col gap-4 rounded-2xl items-center justify-start bg-black/20 backdrop-blur-sm border border-cyan-500/30 shadow-lg shadow-cyan-500/10">
            <div className="flex items-center justify-between w-full">
                <div className="text-lg font-semibold flex items-center justify-center gap-2">
                    {" "}
                    <MdSchool className="text-3xl text-cyan-500" /> Certifications
                </div>
                <div className="flex items-center justify-center gap-2">
                    ...
                    <span>{profileLoading ? "--" : profile?.certifications.length || 0}</span>
                </div>
            </div>
                    ...
                    {profileLoading ? (
                        Array.from({ length: 2 }).map((_, i) => (
                            <CarouselItem key={i} className="h-52">
                                <div className="h-full p-4 border rounded-xl flex flex-col m-1 border-cyan-400 animate-pulse">
                                    {/* Skeleton header */}
                                    <div className="flex justify-between items-start">
                                        <div className="h-5 w-36 bg-gray-700 rounded-md" />
                                        <div className="h-5 w-5 bg-gray-700 rounded-full" />
                                    </div>

                                    {/* Skeleton issuer badge */}
                                    <div className="h-4 w-24 bg-cyan-900/40 rounded-md self-start mt-2 mb-2" />
                                    ...
                                    <div className="mt-auto mb-2 flex flex-wrap gap-2">
                                        <div className="h-4 w-28 bg-cyan-900/40 rounded-md" />
                                        <div className="h-4 w-28 bg-cyan-900/40 rounded-md" />
                                    </div>
                            ...
                            {profile?.certifications.map(certification => (
                                <CarouselItem key={certification.id} className="h-52">
                                    <div className="h-full p-4 border rounded-xl flex flex-col m-1 bg-gradient-to-b from-cyan-950/30 to-black/40 border-cyan-400 shadow-md shadow-cyan-500/20 relative group">
                                        ...
                                        <div className="flex justify-between items-start">
                                            <h4 className="text-md font-medium text-cyan-300 mb-1">
                                                {certification.title}
                                            </h4>
